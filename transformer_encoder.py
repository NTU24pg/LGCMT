...
def attention(query, key, value, mask=None, dropout=None):
    d_k = query.size(-1)
    query_norm = query / (query.norm(dim=-1, keepdim=True) + 1e-9)
    key_norm = key / (key.norm(dim=-1, keepdim=True) + 1e-9)
